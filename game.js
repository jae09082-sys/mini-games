...
function startGame() {
  ...
  cells.forEach(cell => { const button = document.createElement('button'); let held = false, hold; button.className = 'cell'; button.setAttribute('role', 'gridcell'); button.setAttribute('aria-label', `행 ${Math.floor(cell.index / size) + 1}, 열 ${cell.index % size + 1}`); button.addEventListener('click', () => { if (!held) openCell(cell.index); held = false; }); button.addEventListener('contextmenu', e => { e.preventDefault(); toggleFlag(cell.index); }); button.addEventListener('pointerdown', e => { if (e.pointerType !== 'mouse') hold = setTimeout(() => { held = true; toggleFlag(cell.index); }, 500); }); ['pointerup','pointercancel','pointerleave'].forEach(type => button.addEventListener(type, () => clearTimeout(hold))); boardEl.append(button); });
