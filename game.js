...
function openCell(i) { if (state.over || cells[i].flag || cells[i].open) return; begin(i); const cell = cells[i]; cell.open = true; state.opened++; if (cell.mine) return lose(); if (cell.around === 0) neighbors(i).forEach(openCell); if (state.opened === cells.length - mines) win(); render(); }
...
function lose() { state.over = true; clearInterval(interval); cells.filter(c => c.mine).forEach(c => c.open = true); faceEl.textContent = '😵'; messageEl.textContent = '앗, 지뢰를 밟았어요. 다시 도전해 보세요!'; messageEl.className = 'message lost'; }
...
function render() { mineCountEl.textContent = String(Math.max(0, mines - state.flags)).padStart(3, '0'); timerEl.textContent = String(seconds).padStart(3, '0'); cells.forEach((cell, i) => { const el = boardEl.children[i]; el.className = `cell ${cell.open ? 'open' : ''} ${cell.flag ? 'flag' : ''} ${cell.open && cell.mine ? 'mine' : ''}`; el.textContent = cell.flag ? '🚩' : cell.open && cell.mine ? '💣' : cell.open && cell.around ? cell.around : ''; if (cell.open && cell.around) el.classList.add(`n${cell.around}`); }); }
